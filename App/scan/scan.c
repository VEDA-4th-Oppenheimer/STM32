 ...
 *   - 예) step_ddeg = 9  (0.9도) 입력 시: 정확히 8펄스(8 * 0.1125 = 0.900도)로 매 줄 정수배 회전.
 *   - 예) step_ddeg = 10 (1.0도) 입력 시: 가장 가까운 9펄스(1.0125도)로 변환하며 절대각에서
 *         누적하므로 180줄에서 정확히 1600펄스(180.0도)에 수렴(절삭 오차 누적 방지). */
