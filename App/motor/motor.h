 ...
 *    ※ EN 은 active-low.
 *
 *    ★ 타이머 주기(ARR)는 **고정이 아니다.** 펄스마다 이 계층이 다시 쓴다
 *      (아래 "가감속 램프"). 즉 축의 속도는 상수가 아니라 이동 구간에 따라
 *      변한다 — 시작/끝은 느리고 가운데는 빠르다.
 *      ⚠️ 프리스케일러와 ARR 은 motor_init() 이 직접 잡는다. CubeMX 가
 ...
 *    도착시키면 그 충격이 사라진다.
 *
 *  ★ 이동 각도는 **하나도 안 바뀐다.** 램프가 건드리는 것은 펄스 사이의
 ...
 *      (팬 격자 1스텝 0.9도 = 8펄스)은 순항에 도달하지 못하고 삼각 프로파일이 된다 —
 *      느리게 출발해 느리게 도착하므로 짧은 이동일수록 더 조용하다.
 *
 *  ⚠️ 램프가 못 막는 것: **목표를 현재 위치로 당겨서 하는 즉시 정지**
 ...
 *    구간이 길어진다. ⚠️ 너무 낮추면 ARR 이 TIM1(16비트)을 넘는다 — 아래
 *    _Static_assert 가 잡는다(최저 약 16pps).
 *
 *  MOTOR_*_CRUISE_PPS — 램프 후 순항 속도. **종전 고정 속도와 같은 값**이다.
 *    즉 이 변경으로 스캔 소요시간이나 라이다 샘플 밀도는 달라지지 않는다.
 *      틸트 400pps = 45도/s. 라이다 100Hz 기준 0.45도/샘플이라 격자 0.9도에
 *        정확히 2샘플이 떨어진다(main.c 의 설계 근거). ⚠️ 이 값을 올리면
 *        샘플 밀도가 그대로 반비례로 묽어진다 — 격자에 구멍이 생긴다.
 *      팬 100pps = 11.25도/s. 팬은 스윕 중 정지해 있어 샘플과 무관하다.
 *
 *  MOTOR_*_ACCEL_PPS2 — 가속도(pps/s). 크게 잡을수록 램프가 짧아 시간 손해가
 *    줄지만, 램프를 넣은 목적 자체가 무뎌진다.
 *    램프가 잡아먹는 펄스 수는  n = (v_cruise^2 - v_start^2) / (2*accel).
 *      팬  (50→100, 600)  : 7펄스(0.8도) / 0.08초
 *      틸트(50→400, 1200) : 66펄스(7.4도) / 0.29초
 *    비용은 이동 양끝에서 한 번씩이다. 틸트 한 줄(180도=1600펄스, 4.0초)이
 *    4.26초가 된다(+6%) — 200줄 스캔이면 +1분 남짓이다. 팬 180도 파킹은
 *    16.0초 → 16.06초로 사실상 그대로다(SCAN_PARK_TIMEOUT_MS 여유 안).
 * ========================================================================= */
