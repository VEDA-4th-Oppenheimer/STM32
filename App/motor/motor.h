 ...
 *        정확히 2샘플이 떨어진다(main.c 의 설계 근거). 주의: 이 값을 올리면
 ...
 *
 *   주의: 가감속 구간(532펄스 = 스윕의 60도)은 순항보다 느려 샘플이 촘촘하다.
 *     즉 위험한 것은 **가운데 120도(순항 구간)** 뿐이다. 빈 셀이 스윕
 *     중앙에 몰려 나오면 이 값이 원인이고, 끝점에 몰리면 다른 문제다.
 *
 *   핵심: 빈 셀이 보이면 이 값을 먼저 의심할 것. 기구 문제로 오인하기 쉽다. */
 ...
 *
 *  주의: 정수 연산이라 나머지가 버려진다. **절대각 변환에만 쓰고 증분에 쓰지 말 것.**
